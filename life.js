...
/*
   ...
   record change in color as a vector with velocity.
   define directional thresholds based upon velocity.
   ...
   cells are valued from 255 = most dead to 0 = most alive

   use the cell value to determine how frequently cells are tested

   system clock counts ticks; cells are tested every +val ticks
   so a live cell would be 0 or every tick, whereas a halfawy 
   would be every 127 ticks, and a low cell would be every 255 ticks.

   this could translate to every *10 miliseconds.

   so instead of a single nexted loop, there would be interval timers
   that would analyze their cells and change their values and redraw
   them upon change. Instead of 255 or however many timers, there would
   be maybe 10 or 20. so 10 or 20 groups of cell values.

   The data structure would probably have to change for efficiency so
   each timer has a list to go through. The cell value changes would
   add and remove cells from the various structures.
